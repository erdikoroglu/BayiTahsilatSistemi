        ...
        <x-card title="Yeni Cari Oluştur" footer="" actions="">
            <form action="{{ route('admin.users.update',$user) }}" method="post">
                ...
                <div class="mb-10">
                    ...
                    <input type="text" class="form-control" name="company_name" autocomplete="new-company" value="{{ $user->company_name }}"/>
                </div>
                <div class="row">
                    <div class="col-6">
                        <div class="mb-10">
                            <label for="" class="required form-label">Vergi Numarası</label>
                            <input type="text" class="form-control" name="tax_number" autocomplete="new-tax" value="{{ $user->tax_number }}"/>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="mb-10">
                            <label for="" class="required form-label">Logo Cari Kodu</label>
                            <input type="text" class="form-control" name="logo_code" autocomplete="new-logo" value="{{ $user->logo_code }}"/>
                        </div>
                    </div>
                </div>
                <div class="mb-10">
                    <label for="" class="required form-label">Firma Adresi</label>
                    <textarea class="form-control" rows="3" name="address" autocomplete="new-address">{{ $user->address }}</textarea>
                </div>
                <div class="mb-10">
                    <label for="" class="required form-label">Yetkili Adı Soyadı</label>
                    <input type="text" class="form-control" name="name" autocomplete="new-name" value="{{ $user->name }}"/>
                </div>
                <div class="row">
                    <div class="col-6">
                        <div class="mb-10">
                            <label for="" class="required form-label">Yetkili Telefon</label>
                            <input type="text" class="form-control" name="phone" autocomplete="new-phone" value="{{ $user->phone }}"/>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="mb-10">
                            <label for="" class="required form-label">Yetkili E-Posta</label>
                            <input type="text" class="form-control" name="email" autocomplete="new-email" value="{{ $user->email }}"/>
                        </div>
                    </div>
                </div>
                <div data-kt-buttons="true" class="row">
                        ...
                        <label class="btn btn-outline btn-outline-dashed btn-active-light-primary d-flex flex-stack text-start p-6 mb-5 active">
                            <div class="d-flex align-items-center me-2">
                                <div class="form-check form-check-custom form-check-solid form-check-primary me-6">
                                    <input class="form-check-input" type="radio" name="installment" {{ $user->installment == 1 ? 'checked' : null }} value="1"/>
                                </div>
                    ...
                    <div class="col-6">
                        <label class="btn btn-outline btn-outline-dashed btn-active-light-primary d-flex flex-stack text-start p-6 mb-5">
                            <div class="d-flex align-items-center me-2">
                                <div class="form-check form-check-custom form-check-solid form-check-primary me-6">
                                    <input class="form-check-input" type="radio" name="installment" {{ $user->installment == 0 ? 'checked' : null }} value="0"/>
                                </div>
                                <div class="flex-grow-1">
                                    <h2 class="d-flex align-items-center fs-3 fw-bold flex-wrap">
                                        Taksit Yapamaz
